...
async function insertAdditionalData() {
  ...
  await db.favoriteMovie.createMany({
    data: [
      ...
      { profileId: profile2.id, movieId: movie4.id },
      { profileId: profile2.id, movieId: movie3.id },
      { profileId: profile2.id, movieId: movie2.id },
      { profileId: profile2.id, movieId: movie1.id },
    ],
  })
}
